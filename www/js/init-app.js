...
var kirjaTemplate;
var kirjaInfoTemplate;
var tehtavatTemplate;
var valitseKirjaTemplate;
var omatKirjatTemplate;
var hakuTemplate;
/*var chooseBookListTemplate;*/
var chooseTaskListTemplate;
...
app.initEvents = function() {
    ...
    /*Functions for choosing book and task list*/
    /*checkIfBookListInLS();*/
    checkIfTaskListInLS();
...
function compileHandlebarTemplates() {
  ...
  var kirjaTemplateSource   = $("#book-entry-template").html(); // Fetch template from index.html using Id
  kirjaTemplate = Handlebars.compile(kirjaTemplateSource); // compile html into a template

  var kirjaInfoTemplateSource   = $("#book-info-template").html();
  kirjaInfoTemplate = Handlebars.compile(kirjaInfoTemplateSource);

  var tehtavatTemplateSource   = $("#tehtavat-template").html();
  tehtavatTemplate = Handlebars.compile(tehtavatTemplateSource);

  var valitseKirjaTemplateSource = $("#valitse-kirja-template").html();
  valitseKirjaTemplate = Handlebars.compile(valitseKirjaTemplateSource);

  var omatKirjaTemplateSource   = $("#omat-kirjat-template").html();
  omatKirjatTemplate = Handlebars.compile(omatKirjaTemplateSource);

  var hakuTemplateSource   = $("#haku-template").html();
  hakuTemplate = Handlebars.compile(hakuTemplateSource);

 /* var chooseBookListTemplateSource   = $("#choose-book-list-template").html();
  chooseBookListTemplate = Handlebars.compile(chooseBookListTemplateSource); */

  var chooseTaskListTemplateSource   = $("#choose-task-list-template").html();
  chooseTaskListTemplate = Handlebars.compile(chooseTaskListTemplateSource);

}

/*Functions for choosing book and task list*/
/*function checkIfBookListInLS () {
  if (localStorage.getItem("selectedBookList") === null) {
    var selectedBookList = "1";
    localStorage.setItem("selectedBookList", JSON.stringify(selectedBookList));
    lsSelectedBookList = JSON.parse(localStorage.getItem("selectedBookList"));
    return lsSelectedBookList;
  } else {
    lsSelectedBookList = JSON.parse(localStorage.getItem("selectedBookList"));
    return lsSelectedBookList;
  }
  ...
  if (localStorage.getItem("selectedTaskList") === null) {
    ...
    lsSelectedTaskList = JSON.parse(localStorage.getItem("selectedTaskList"));
    return lsSelectedTaskList;
  } else {
    lsSelectedTaskList = JSON.parse(localStorage.getItem("selectedTaskList"));
    return lsSelectedTaskList;
  }
